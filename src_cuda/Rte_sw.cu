...
void Rte_sw_gpu::rte_sw(
        ...
        const Array_gpu<FloatFlux,2>& inc_flux_dir,
        const Array_gpu<FloatSurface ,2>& sfc_alb_dir,
        const Array_gpu<FloatSurface,2>& sfc_alb_dif,
        const Array_gpu<FloatFlux,2>& inc_flux_dif,
        Array_gpu<FloatFlux,3>& gpt_flux_up,
        Array_gpu<FloatFlux,3>& gpt_flux_dn,
        Array_gpu<FloatFlux,3>& gpt_flux_dir)
{
    const int ncol = optical_props->get_ncol();
    const int nlay = optical_props->get_nlay();
    const int ngpt = optical_props->get_ngpt();

    Array_gpu<FloatSurface,2> sfc_alb_dir_gpt({ncol, ngpt});
    Array_gpu<FloatSurface,2> sfc_alb_dif_gpt({ncol, ngpt});
    ...
    const FloatFlux* inc_flux_dif_ptr = (inc_flux_dif.size() == 0) ? nullptr : inc_flux_dif.ptr();
