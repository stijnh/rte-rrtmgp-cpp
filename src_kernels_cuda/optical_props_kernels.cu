...
void increment_2stream_by_2stream_kernel(
            ...
            TAU_TYPE* __restrict__ tau1, Float* __restrict__ ssa1, Float* __restrict__ g1,
            const TAU_TYPE* __restrict__ tau2, const Float* __restrict__ ssa2, const Float* __restrict__ g2)
{
    const int icol = blockIdx.x*blockDim.x + threadIdx.x;
    const int ilay = blockIdx.y*blockDim.y + threadIdx.y;
    const int igpt = blockIdx.z*blockDim.z + threadIdx.z;
