        ...
        gas_concs_subset(std::make_unique<Gas_concs_gpu<TF>>(gas_concs, 1, ncols))
{
    ...
    if(memory_pool)
    {
// TODO: Enable the lock, figure out why not enough is allocated
        ...
        radiation_solver_work_arrays_gpu<TF>* work_arrays) const
{
        ...
        radiation_block_work_arrays_gpu<TF>* work_block)
    {
        ...
        if (switch_cloud_optics)
        {
            ...
            // Add the cloud optical props to the gas optical properties.
            add_to(
                dynamic_cast<Optical_props_2str_gpu<TF>&>(*(work->sw_optical_props_subset)),
                dynamic_cast<Optical_props_2str_gpu<TF>&>(*(work->sw_cloud_optical_props_subset)));
        }
