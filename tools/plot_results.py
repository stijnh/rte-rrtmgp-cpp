...
def plot_times(name, results, times):
    plt.figure(figsize=(figsize[0], figsize[1]*0.8))

    baseline = sum(times["double"])
    time = np.array([sum(times[k]) for k in labels])
    ...
    ylim = np.ceil(baseline / 1000.0 + 1) * 1000
...
plot_times("leonardo", errors_a100, times_leonardo)
plot_times("lumi", errors_lumi, times_lumi)
